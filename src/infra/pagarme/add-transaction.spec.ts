...
describe('AddTransaction Pagarme', () => {
  const makeSut = (): AddTransaction => new AddTransaction();

  test('Should return an a transaction on success', async () => {
    const sut = makeSut();
    const transaction = await sut.add(makeFakeRequest());
